...
public class CommunityPartnerController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @RequestMapping(value = "/api/communityPartner/getById/{id}", method = RequestMethod.GET)
    public CommunityPartner findById(@PathVariable Long id) {
        return communityPartnerService.findById(id);
    }

    @PreAuthorize("hasRole('ADMIN')")
    @RequestMapping(value = "api/communityPartner/{id}", method = RequestMethod.DELETE)
    public boolean delete(@PathVariable Long id) {
        return communityPartnerService.delete(id);
    }
}