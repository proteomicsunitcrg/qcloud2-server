...
public class ThresholdForPlotFactory {
	...
	public static ThresholdForPlotImpl create(Threshold threshold) {
		...
		threshold.getThresholdParams().forEach(tp -> {
			params.add(new ParamsNoThresholdImpl(tp.getStepValue(), tp.getInitialValue(), tp.getContextSource(),
					tp.getIsEnabled()));
		});

		return new ThresholdForPlotImpl(threshold.getThresholdType(), threshold.getDirection(),
